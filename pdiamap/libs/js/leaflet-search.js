...
L.Control.Search = L.Control.extend({
	...
	//
	//Managed Events:
	//	search_locationfound	{latlng, tienda, layer} fired after moved and show markerLocation
	...
	options: {
		...
		propertyLoc: 'loc',			//field for remapping location, using array: ['latname','lonname'] for select double fields(ex. ['lat','lon'] ) support dotted format: 'prop.subprop.tienda'
		propertyName: 'tienda',		//property in marker.options(or feature.properties for vector layer) trough filter elements in layer,
		...
		textCancel: 'Cancel',		//tienda in cancel button		
		textPlaceholder: 'Store ID 1...',//placeholder value			
...
////start DOM creations
	...
	_createCancel: function (tienda, className) {
		var cancel = L.DomUtil.create('a', className, this._container);
		cancel.href = '#';
		cancel.tienda = tienda;
	...
	_createButton: function (tienda, className) {
		var button = L.DomUtil.create('a', className, this._container);
		button.href = '#';
		button.tienda = tienda;
...
//	in this way every record can have a free structure of attributes, only 'loc' is required
	...
	showLocation: function(latlng, tienda) {	//set location on map from _recordsCache
		...
		if(this._markerLoc)
		{
			...
			this._markerLoc.settienda(tienda);
...
L.Control.Search.Marker = L.Marker.extend({
	...
	settienda: function(tienda) {
		tienda = tienda || '';
		this.options.tienda = tienda;
		if(this._icon)
			this._icon.tienda = tienda;
		return this;
	},
